...
const SubmitFormPage: React.FC = () => {
  ...
  const [
    submitResponse,
    { isLoading: isSubmitting, isSuccess: isSubmitSuccess, error: submitError },
    ...
    if (submitError) {
      showSnackbar(
        `Error submitting feedback: ${
          (submitError as any).data?.message || (submitError as any).error
        }`,
        "error"
      );
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    form?.questions.forEach((q) => {
      if (q.required) {
        ...
        if (
          !answer ||
          (q.type === "text" && !answer.answerText?.trim()) ||
            ...
            {form.questions.map((q: Question) => (
              <Paper key={q.id} elevation={2} sx={{ p: 3, borderRadius: 2 }}>
                <Typography
                  variant="h6"
                  component="label"
                  sx={{ mb: 2, display: "block", fontWeight: "medium" }}
                >
                  {q.questionText}{" "}
